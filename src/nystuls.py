...
def Print_Inventory(inventory):
    ...
    for x in xrange(len(inventory)):
        ...
        if item['type'] != last_type:
            if (last_type == 'Wands' or last_type == "Rods & Staves") and (item['type'] == 'Wands' or item['type'] == "Rods & Staves"):
                pass
            else:
        ...
        if item['gplo'] > 0:
            if item['cnt'] > 1:
               logging.info( "{:3d}: {:50}{:5d} GP  {:3d} count".format(x,item['name'],int(item['gplo']),int(item['cnt'])))
            elif item['cnt'] == 1:
